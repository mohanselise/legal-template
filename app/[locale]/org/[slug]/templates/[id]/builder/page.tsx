...
import { TypeformBuilder, type Template, type ScreenWithFields } from "@/app/[locale]/admin/templates/[id]/builder/_components/typeform-builder";

export default function OrgFormBuilderPage() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      // Now fetch template and screens using org API
      const [templateRes, screensRes] = await Promise.all([
        fetch(`/api/org/${currentOrgId}/templates/${templateId}`),
        fetch(`/api/org/${currentOrgId}/templates/${templateId}/screens`),
      ]);
  ...
  return (
    <TypeformBuilder
      ...
      locale={locale}
      apiBasePath={`/api/org/${orgId}`}
      backUrl={`/${locale}/org/${orgSlug}/templates/${templateId}/edit`}
    />
  );
}
