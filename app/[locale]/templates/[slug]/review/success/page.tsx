...
export default function SignatureSuccessPage() {
  ...
  const payload = useMemo<PayloadSummary | null>(() => {
    ...
    // Try to get from template review storage first
    const reviewData = loadTemplateReview(slug);
    if (reviewData) {
      return {
        document: reviewData.document,
        formData: reviewData.formData,
        signatories: reviewData.document?.signatories?.map(sig => ({
          name: sig.name,
          email: sig.email || '',
  ...
  // Redirect if no payload found
  useEffect(() => {
    if (!payload && typeof window !== 'undefined') {
      // Redirect to template page if no data found
      router.push(`/${locale}/templates/${slug}`);
    }
  }, [payload, router, locale, slug]);

  // Get document title and first signatory name
  const documentTitle = 
    payload?.templateTitle || 
    payload?.document?.metadata?.title || 
    'Legal Document';
  
  const firstSignatoryName: string =
    payload?.signatories?.[0]?.name ||
    payload?.document?.signatories?.[0]?.name ||
    'the signatory';

  const signatories = payload?.signatories
    ? [...payload.signatories].sort((a, b) => (a.order ?? 0) - (b.order ?? 0))
    : payload?.document?.signatories?.map(sig => ({
        name: sig.name,
        email: sig.email || '',
        role: sig.title || sig.party,
        order: 0,
    ...
    <div className="min-h-screen bg-[hsl(var(--bg))] text-[hsl(var(--fg))] flex flex-col font-body">
      ...
      <main className="flex-1 container mx-auto px-4 py-12 max-w-5xl">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
          ...
          {/* Left Column: Success State & Document Info */}
          <div className="lg:col-span-7 space-y-8">
            ...
            <div className="space-y-4">
              ...
              <h1 className="font-heading text-4xl sm:text-5xl font-bold text-[hsl(var(--fg))] leading-tight">
                {documentTitle} for <span className="text-[hsl(var(--selise-blue))]">{firstSignatoryName}</span> is on its way.
              </h1>
                  ...
                  {signatories.map((signatory, idx) => (
                    <div key={idx} className="flex items-center gap-4 p-4 hover:bg-muted/50 transition-colors">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {signatory.role && (
                          <p className="text-xs text-muted-foreground truncate">{signatory.role}</p>
                        )}
                      </div>
          ...
          {/* Right Column: Signup Conversion Card */}
                ...
                <Card className="relative border-[hsl(var(--selise-blue))]/30 shadow-xl overflow-hidden bg-white dark:bg-[hsl(var(--card))]">
                  ...
                  <div className="bg-gradient-to-br from-[hsl(var(--selise-blue))]/10 to-[hsl(var(--sky-blue))]/10 p-8 text-center border-b border-[hsl(var(--selise-blue))]/10">
                    ...
                    <p className="text-muted-foreground text-sm leading-relaxed">
                      Create a free account to track this document and manage your contracts in one place.
                    </p>
                  </div>
                    ...
                    <ul className="space-y-4">
                      <li className="flex items-start gap-3">
                        ...
                        <div>
                          ...
                          <span className="text-xs text-muted-foreground">See when signatories open and sign.</span>
                        </div>
                      </li>
                      <li className="flex items-start gap-3">
                        <div className="p-1.5 rounded-lg bg-[hsl(var(--selise-blue))]/10 mt-0.5">
